...
VAD_STATE vad(VAD_DATA *vad_data, float *x) {
  ...
  case ST_SILENCE:
    if (f.p > vad_data->p0 + vad_data->alfa0) //f.p ->  f:feature   p:potencia, si la potencia es mayor 0.95 -> pasamos a voz
      vad_data->state = ST_VOICE;
    break;

  case ST_VOICE:
    if (f.p < vad_data->p0 + vad_data->alfa0) // si la potencia es menor que 0.1, es silenciome
      vad_data->state = ST_SILENCE;
    break;
